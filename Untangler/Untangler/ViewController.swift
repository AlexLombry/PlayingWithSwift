...
class ViewController: UIViewController {
    ...
    private func levelUp() {
        currentLevel += 1
        // Fresh view every single time
        connections.forEach { $0.removeFromSuperview() }
        connections.removeAll()
        
        for _ in 1...(currentLevel + 4) {
            // CGSize of 44 is the minimum required by Apple to handle Touch simply
    ...
    /// Position handling
    private func place(_ connection: UIView) {
    ...
    /// Handle linecrossing (from hackingwithswift.com)
